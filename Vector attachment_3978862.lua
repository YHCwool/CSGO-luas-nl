...
local vector_origin = (function()
    ...
    local pClientEntityList = utils.create_interface("client.dll", "VClientEntityList003") or error("invalid interface", 2)
...
local infobox = function()
    ...
    renderer.rectangle(easing.offset[1] + 54, easing.offset[2] + 40 + easing.tts[3], eased_tts, 2, 255, 255, 255, easing.tts[2])

    renderer.text(easing.offset[1] + 54, easing.offset[2] + 33, 255, 255, 255, 255, "-", 0, "STAT")
    renderer.text(easing.offset[1] + 74, easing.offset[2] + 33, easing.color_1[1], easing.color_1[2], easing.color_1[3], 255, "-", 0, (ready and "ON") or "OFF")
    renderer.text(easing.offset[1] + 100, easing.offset[2] + 33, 255, 255, 255, (BOT and 180) or 150, "-", 0, (BOT and "BO") or "MA")

    for i = 1, 4, 1 do
        local offset = (i * 15) + 39
        local text = (i == 1 and "AP") or (i == 2 and "PR") or (i == 3 and "BT") or (i == 4 and "SP")
        renderer.text(easing.offset[1] + offset, easing.offset[2] + 43 + easing.tts[3], easing.ind_r[i], easing.ind_g[i], easing.ind_b[i], easing.ind_a[i], "-", 0, text)
    end
end
